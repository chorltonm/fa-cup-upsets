CREATE OR REPLACE VIEW `birkbeck-msc-project-422917.analysis_layer.view_round_3_fa_cup_matches` AS
...
SELECT
  ...
  IFNULL(vrh.rivalry_flag,vra.rivalry_flag) AS rivalry_flag,
  CASE 
...
LEFT JOIN analysis_layer.view_round_3_fa_cup_rank rankh ON vmtc.season_year = rankh.season_year AND vmtc.match_home_team_id = rankh.team_id

-- Away Team Round 3 League Position
LEFT JOIN analysis_layer.view_round_3_fa_cup_rank ranka ON vmtc.season_year = ranka.season_year AND vmtc.match_away_team_id = ranka.team_id

-- Does rivalry exist home away teams ids

LEFT JOIN transform_layer.view_rivalries vrh ON vmtc.match_home_team_id = vrh.team1_id AND vmtc.match_away_team_id = vrh.team2_id

-- Does rivalry exist away home teams ids

LEFT JOIN transform_layer.view_rivalries vra ON vmtc.match_home_team_id = vra.team2_id AND vmtc.match_away_team_id = vra.team1_id
